...
@section('content')
    ...
    <div style="min-height:75vh; display:flex; flex-direction:column; align-items:center; justify-content:center;">
        ...
        {{-- Inline error list --}}
        @if($errors->any())
        <div style="margin-bottom:20px; background:#fff5f5; border:1px solid #fecaca; border-radius:12px; padding:14px 18px; width:100%; max-width:640px;">
        ...
        <div style="background:#fff; border:1px solid #e2e8f0; border-radius:18px; padding:32px; width:100%; max-width:640px; box-shadow:0 4px 24px rgba(15,23,42,0.06);">
            <form action="{{ route('books.update', $books->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div style="margin-bottom:16px;">
                    ...
                    <input type="text" name="judul" value="{{ old('judul', $books->judul) }}" required
                        style="width:100%; background:{{ $errors->has('judul') ? '#fff5f5' : '#f8fafc' }}; border:1px solid {{ $errors->has('judul') ? '#fca5a5' : '#e2e8f0' }}; color:#1e293b; border-radius:11px; padding:10px 14px; font-size:0.875rem; outline:none; font-family:'Outfit',sans-serif;"
                        onfocus="this.style.borderColor='#3b82f6';this.style.background='#fff';"
                        onblur="this.style.borderColor='{{ $errors->has('judul') ? '#fca5a5' : '#e2e8f0' }}';this.style.background='{{ $errors->has('judul') ? '#fff5f5' : '#f8fafc' }}';" />
                </div>
                ...
                <div style="margin-bottom:16px;">
                    ...
                    <textarea name="sinopsis" rows="3" required
                        style="width:100%; background:{{ $errors->has('sinopsis') ? '#fff5f5' : '#f8fafc' }}; border:1px solid {{ $errors->has('sinopsis') ? '#fca5a5' : '#e2e8f0' }}; color:#1e293b; border-radius:11px; padding:10px 14px; font-size:0.875rem; outline:none; font-family:'Outfit',sans-serif; resize:vertical;"
                        onfocus="this.style.borderColor='#3b82f6';this.style.background='#fff';"
                        onblur="this.style.borderColor='#e2e8f0';this.style.background='#f8fafc';">{{ old('sinopsis', $books->sinopsis) }}</textarea>
                </div>
                ...
                <div style="margin-bottom:16px;">
                    ...
                    <input type="number" name="tahun_terbit" value="{{ old('tahun_terbit', $books->tahun_terbit) }}" required
                        style="width:100%; background:{{ $errors->has('tahun_terbit') ? '#fff5f5' : '#f8fafc' }}; border:1px solid {{ $errors->has('tahun_terbit') ? '#fca5a5' : '#e2e8f0' }}; color:#1e293b; border-radius:11px; padding:10px 14px; font-size:0.875rem; outline:none; font-family:'Outfit',sans-serif;"
                        onfocus="this.style.borderColor='#3b82f6';this.style.background='#fff';"
                ...
                <div style="margin-bottom:16px;">
                    ...
                    <select name="genre_id"
                        style="width:100%; background:#f8fafc; border:1px solid #e2e8f0; color:#1e293b; border-radius:11px; padding:10px 14px; font-size:0.875rem; outline:none; font-family:'Outfit',sans-serif; cursor:pointer;"
                        onfocus="this.style.borderColor='#3b82f6';this.style.background='#fff';"
                        onblur="this.style.borderColor='#e2e8f0';this.style.background='#f8fafc';">
                        <option value="">-- Pilih Genre --</option>
                        @foreach($genres as $item)
                        <option value="{{ $item->id }}" {{ old('genre_id', $books->genre_id) == $item->id ? 'selected' : '' }}>
                            {{ $item->name_genres }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div style="margin-bottom:16px;">
                    ...
                    <select name="author_id"
                        ...
                        <option value="{{ $key->id }}" {{ old('author_id', $books->author_id) == $key->id ? 'selected' : '' }}>
                            {{ $key->name_author }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div style="margin-bottom:20px;">
                    ...
                    <div style="margin-bottom:10px;">
                        ...
                        <img src="{{ asset('storage/' . $books->image) }}" alt="Cover"
                            style="width:80px; height:110px; object-fit:cover; border-radius:10px; border:1px solid #e2e8f0;">
                    </div>
                    @endif

                    <label style="display:flex; align-items:center; gap:10px; background:#f8fafc; border:1px dashed #cbd5e1; border-radius:11px; padding:12px 14px; cursor:pointer; transition:border 0.17s;"
                    ...
                    <img src="" id="previewImg" alt="Preview"
                        style="margin-top:10px; width:80px; height:110px; object-fit:cover; border-radius:10px; border:1px solid #e2e8f0; display:none;" />
                </div>

                {{-- Buttons --}}
                <div style="display:flex; align-items:center; gap:10px; padding-top:4px;">
                    <button type="submit"
                        style="background:linear-gradient(135deg,#3b82f6,#60a5fa); color:#fff; font-weight:600; font-size:0.855rem; padding:10px 22px; border-radius:11px; border:none; cursor:pointer; box-shadow:0 4px 14px rgba(59,130,246,0.3); font-family:'Outfit',sans-serif;"
                        onmouseover="this.style.opacity='0.9'" onmouseout="this.style.opacity='1'">
                        Simpan Perubahan
                    </button>
                    <a href="{{ route('books.index') }}"
                        style="font-size:0.855rem; color:#94a3b8; padding:10px 18px; border-radius:11px; text-decoration:none; border:1px solid #e2e8f0; transition:all 0.17s;"
